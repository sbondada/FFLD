...
vector<Rect> detectObjects(Mat image, char * model,	vector<Rect> & bndboxes)
{
...
int main(int argc, char * argv[])
{
	...
	const size_t lastDotim = fileim.find_last_of('.');
	const size_t lastDotmod = filemod.find_last_of('.');
	Mat image;
    BackgroundSubtractorMOG mog;
    if (fileim.substr(lastDotim) == ".jpg" && filemod.substr(lastDotmod) == ".txt") 
    {
