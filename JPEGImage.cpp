...
//read image matrix and store in JPEGImage format
JPEGImage::JPEGImage(cv::Mat &image) 
{
	
	vector<uint8_t> bits(image.cols * image.rows * (image.dims+1));
    ...
    for(int i = 0; i < image.rows; i++)
    {
        ...
        {
            for(int k = 0; k < image.dims + 1; k++)
            {
                bits[i*image.cols*(image.dims+1) + j*(image.dims+1) + k] = PixelPtr[i*image.cols*(image.dims+1) + j*(image.dims+1) + k];

                //Scalar_<uint8_t> bgrPixel;
                //bits = pixelPtr[i*foo.cols*cn + j*cn + 0]; // B
